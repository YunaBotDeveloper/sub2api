...
type Group struct {
	...
	// 是否禁止此 OpenAI/Composite 分组请求使用 Fast（移除 service_tier）
	DisableOpenaiFast bool `json:"disable_openai_fast,omitempty"`
	// 是否强制此 OpenAI/Composite 分组请求使用 Ultrafast（service_tier=ultrafast）
	ForceOpenaiUltrafast bool `json:"force_openai_ultrafast,omitempty"`
	...
	// 分组并发上限，0 表示不限制；大于 0 时接管该分组用户的并发
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldDisableOpenaiFast:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field disable_openai_fast", values[i])
			} else if value.Valid {
				_m.DisableOpenaiFast = value.Bool
			}
		case group.FieldForceOpenaiUltrafast:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field force_openai_ultrafast", values[i])
			} else if value.Valid {
				_m.ForceOpenaiUltrafast = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("disable_openai_fast=")
	builder.WriteString(fmt.Sprintf("%v", _m.DisableOpenaiFast))
	builder.WriteString(", ")
	builder.WriteString("force_openai_ultrafast=")
	builder.WriteString(fmt.Sprintf("%v", _m.ForceOpenaiUltrafast))
	builder.WriteString(", ")
