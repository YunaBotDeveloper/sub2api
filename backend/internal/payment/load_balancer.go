...
// enabling cross-provider load balancing across several accounts of one gateway.
...
func getInstanceChannelLimits(inst *dbent.PaymentProviderInstance, paymentType PaymentType) ChannelLimits {
	...
	if cl, ok := limits[paymentType]; ok {
		return cl
	}
	// Fall back to the gateway-wide entry so an instance can cap every one of
	// its sub-methods with a single limits key.
	if cl, ok := limits[inst.ProviderKey]; ok {
		return cl
	}
	return ChannelLimits{}
}
...
// normalizeVisibleMethodSupportType 归一化面板与实例配置里的支付方式写法。
