export default {
    settings: {
      ...
      payment: {
        ...
        subscriptionUsdToCnyRate: '订阅换算汇率',
        subscriptionUsdToCnyRateHint:
          '套餐每 1 USD 价格收取多少结算币种（如 25000 VND）。0 或留空 = 不换算，订阅按 price 数值直接收款。启用后所有套餐 price 必须按 USD 定价',
        subscriptionUsdToCnyRateDisabled: '未启用（按 price 直付）',
        rechargeFeeRate: '充值手续费率',
        rechargeFeeRateHint: '用户充值时额外收取的手续费百分比，0 表示不收取手续费',
        exchangeRateMarkup: '汇率加价',
        exchangeRateMarkupHint: 'USD 与网关结算币种互换时，在越南外贸银行卖出价之上叠加的百分比。0 表示按牌价原价换算。',
        exchangeRateMaxAge: '汇率缓存最长可用时长（小时）',
        exchangeRateMaxAgeHint: '牌价接口取不到时，最多沿用多旧的缓存汇率；超过这个时长就拒绝建单，而不是拿一个不知道多旧的价格收钱。默认 24。',
        ...
        nowPaymentsWebhookHint: '在 NOWPayments 后台（Settings -> Payments -> IPN）把下面的地址配置为 IPN 回调地址，并把它生成的 IPN Secret 填到下方：',
        field_nowPaymentsIpnSecretKeyHint: '必填。回调只有在 x-nowpayments-sig 签名与该密钥吻合时才被接受——NOWPayments 没有可用来二次确认订单的查询接口。',
