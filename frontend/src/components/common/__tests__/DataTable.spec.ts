...
describe('DataTable', () => {
  ...
  it('renders paired sort arrows and highlights the active direction', async () => {
    ...
    expect(nameHeader.findAll('svg')[0].classes()).toContain('text-accent')
    expect(nameHeader.findAll('svg')[1].classes()).toContain('text-border-strong')

    await nameHeader.trigger('click')
    await wrapper.vm.$nextTick()

    expect(nameHeader.attributes('aria-sort')).toBe('descending')
    expect(nameHeader.findAll('svg')[0].classes()).toContain('text-border-strong')
    expect(nameHeader.findAll('svg')[1].classes()).toContain('text-accent')
  })
