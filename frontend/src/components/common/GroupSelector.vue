<template>
  <div>
    <label class="input-label">
      ...
      <span class="font-normal text-fg-subtle">{{ t('common.selectedCount', { count: modelValue.length }) }}</span>
    </label>
    <div
      v-if="isSearchable"
      class="flex items-center gap-2 rounded-t-lg border border-b-0 border-border bg-surface-sunken px-3 py-2"
    >
      <Icon name="search" size="sm" class="shrink-0 text-fg-subtle" />
      <input
        v-model="searchText"
        type="text"
        :placeholder="t('common.searchPlaceholder')"
        class="flex-1 bg-transparent text-sm text-fg placeholder:text-fg-subtle focus:outline-none"
      />
    </div>
    <div
      :class="[
        ...
        isSearchable
          ? 'rounded-b-lg border border-t-0 border-border bg-surface-sunken'
          : 'rounded-sm border border-border bg-surface-sunken'
      ]"
    >
      <label
      ...
      >
        <input
          ...
          class="h-3.5 w-3.5 shrink-0 rounded border-border-strong text-accent focus:ring-accent"
        />
        <GroupBadge
          :name="group.name"
        ...
        />
        <span class="shrink-0 text-xs text-fg-subtle">{{ group.account_count || 0 }}</span>
      </label>
      <div
        v-if="filteredGroups.length === 0"
        class="col-span-2 py-2 text-center text-sm text-fg-muted"
      >
