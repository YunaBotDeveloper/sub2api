  ...
  <div>
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        ...
        >
          <div
            class="w-full max-w-[620px] overflow-hidden rounded-lg bg-white shadow-2xl ring-1 ring-black/5 dark:bg-dark-800 dark:ring-white/10"
            @click.stop
          >
            <!-- Header with Gradient -->
            <div class="relative overflow-hidden border-b border-gray-100/80 bg-accent-50/50 px-6 py-5 dark:border-dark-700/50 dark:bg-accent-900/10">
              <div class="relative z-10 flex items-start justify-between">
                <div>
                  <div class="flex items-center gap-2">
                    <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-accent-500 text-white shadow-lg">
                  ...
                  <p v-if="unreadCount > 0" class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    <span class="font-medium text-accent-600 dark:text-accent-400">{{ unreadCount }}</span>
                ...
                <div class="flex items-center gap-2">
                  <button
                    ...
                    @click="markAllAsRead"
                    :disabled="loading"
                    class="rounded-lg bg-accent-600 px-4 py-2 text-xs font-medium text-white shadow-lg transition-all hover:bg-accent-700 disabled:opacity-50 dark:bg-accent-500 dark:hover:bg-accent-600"
                  >
              ...
              <!-- Decorative gradient -->
              <div class="absolute right-0 top-0 h-full w-48 bg-accent-100/20 dark:bg-accent-900/10"></div>
            </div>
            ...
            <div class="max-h-[65vh] overflow-y-auto">
              ...
              <div v-if="loading" class="flex items-center justify-center py-16">
                <div class="relative">
                  <div class="h-12 w-12 animate-spin rounded-full border-4 border-gray-200 border-t-accent-600 dark:border-dark-600 dark:border-t-accent-400"></div>
                  <div class="absolute inset-0 h-12 w-12 animate-pulse rounded-full border-4 border-accent-400/30"></div>
                </div>
              </div>
              ...
              <div v-else-if="announcements.length > 0">
                <div
                  ...
                  :class="{ 'bg-accent-50/30 dark:bg-accent-900/5': !item.read_at }"
                ...
                >
                  ...
                  <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center">
                    <div
                      v-if="!item.read_at"
                      class="relative flex h-10 w-10 items-center justify-center rounded-xl bg-accent-500 text-white shadow-lg"
                    >
                      <!-- Pulse ring -->
                      <span class="absolute inline-flex h-full w-full animate-ping rounded-xl bg-accent-400 opacity-75"></span>
                      <!-- Icon -->
                  ...
                  <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="mt-1 flex items-center gap-2">
                        ...
                        <span
                          v-if="!item.read_at"
                          class="inline-flex items-center gap-1 rounded-md bg-accent-100 px-1.5 py-0.5 text-xs font-medium text-accent-700 dark:bg-accent-900/40 dark:text-accent-300"
                        >
                          <span class="relative flex h-1.5 w-1.5">
                            <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-accent-500 opacity-75"></span>
                            <span class="relative inline-flex h-1.5 w-1.5 rounded-full bg-accent-600"></span>
                          </span>
                          {{ t('announcements.unread') }}
                        </span>
                      </div>
              ...
              <div v-else class="flex flex-col items-center justify-center py-16">
                <div class="relative mb-4">
                  ...
                  <div class="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-success-500 text-white">
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        ...
        >
          <div
            class="w-full max-w-[780px] overflow-hidden rounded-lg bg-white shadow-2xl ring-1 ring-black/5 dark:bg-dark-800 dark:ring-white/10"
            @click.stop
          >
            <!-- Header with Decorative Elements -->
            <div class="relative overflow-hidden border-b border-gray-100 bg-accent-50/80 px-8 py-6 dark:border-dark-700 dark:bg-accent-900/20">
              <!-- Decorative background elements -->
              <div class="absolute right-0 top-0 h-full w-64 bg-accent-100/30 dark:bg-accent-900/20"></div>
              <div class="absolute -right-8 -top-8 h-32 w-32 rounded-full bg-accent-400/20 blur-3xl"></div>
              <div class="absolute -left-4 -bottom-4 h-24 w-24 rounded-full bg-gray-400/20 blur-2xl"></div>

              <div class="relative z-10 flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  <!-- Icon and Category -->
                  <div class="mb-3 flex items-center gap-2">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-accent-500 text-white shadow-lg">
                      <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                    </div>
                    <div class="flex items-center gap-2">
                      <span class="rounded-lg bg-accent-100 px-2.5 py-1 text-xs font-medium text-accent-700 dark:bg-accent-900/40 dark:text-accent-300">
                        {{ t('announcements.title') }}
                      </span>
                      <span
                        v-if="!selectedAnnouncement.read_at"
                        class="inline-flex items-center gap-1.5 rounded-lg bg-accent-500 px-2.5 py-1 text-xs font-medium text-white shadow-lg"
                      >
                ...
                <button
                  @click="closeDetail"
                  class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-white/50 text-gray-500 transition-all hover:bg-white hover:text-gray-700 dark:bg-dark-700/50 dark:text-gray-400 dark:hover:bg-dark-700 dark:hover:text-gray-300"
                  :aria-label="t('common.close')"
                >
                  <Icon name="x" size="md" />
                </button>
            ...
            <div class="max-h-[60vh] overflow-y-auto bg-white px-8 py-8 dark:bg-dark-800">
              ...
              <div class="relative">
                ...
                <div class="absolute left-0 top-0 bottom-0 w-1 rounded-full bg-accent-500"></div>
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <button
                    ...
                    class="rounded-xl bg-accent-600 px-5 py-2.5 text-sm font-medium text-white shadow-lg transition-all hover:scale-105"
                  >
