<template>
  <div
    ...
    class="flex flex-shrink-0 items-center justify-end gap-2 border-b border-border px-4 py-2"
  >
    <span v-if="pendingCount > 0" class="text-xs text-fg-muted">
      {{ t('usage.ipGeo.pending', { count: pendingCount }) }}
    </span>
    <button
      type="button"
      class="inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-medium text-accent transition-colors hover:bg-accent-weak disabled:cursor-not-allowed disabled:opacity-50"
