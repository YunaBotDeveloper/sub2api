  ...
  >
    <div v-if="user" class="space-y-4">
      <div
        ...
        class="border border-warning/40 bg-warning-weak px-3 py-2 text-sm text-warning-strong"
      >
      ...
      <div v-if="loading" class="py-10 text-center text-fg-muted">{{ t('common.loading') }}</div>
      <div v-else>
        ...
        <table class="table min-w-full">
          ...
          <tbody>
            <tr v-for="row in quotas" :key="row.platform">
              <td class="font-mono text-fg">{{ row.platform }}</td>
              <td>
                <div class="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    class="text-xs text-fg-subtle hover:text-warning disabled:opacity-50"
              ...
              <td class="whitespace-nowrap text-right text-xs tabular-nums text-fg-muted">
        ...
        <p class="mt-3 text-xs text-fg-muted">{{ t('admin.users.platformQuota.hint') }}</p>
        <div class="mt-3">
