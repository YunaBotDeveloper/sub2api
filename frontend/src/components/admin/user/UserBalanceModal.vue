<template>
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      <div class="flex items-center gap-3 border-b-2 border-accent pb-3">
        <div class="flex-1"><p class="font-medium text-fg">{{ user.email }}</p><p class="text-sm text-fg-muted">{{ t('admin.users.currentBalance') }}: <span class="font-semibold tabular-nums text-fg">${{ formatBalance(user.balance) }}</span></p></div>
      </div>
      <div>
        <label class="input-label">{{ operation === 'add' ? t('admin.users.depositAmount') : t('admin.users.withdrawAmount') }}</label>
        <div class="relative flex gap-2">
          <div class="relative flex-1"><div class="absolute left-3 top-1/2 -translate-y-1/2 font-medium text-fg-muted">$</div><input v-model.number="form.amount" type="number" step="any" min="0" required class="input pl-8 text-right tabular-nums" /></div>
      ...
      <div v-if="form.amount > 0" class="border-t-[3px] border-double border-fg pt-3"><div class="flex items-center justify-between text-sm"><span class="text-fg">{{ t('admin.users.newBalance') }}:</span><span class="text-h2 font-bold tabular-nums text-fg">${{ formatBalance(calculateNewBalance()) }}</span></div></div>
    </form>
    <template #footer>
