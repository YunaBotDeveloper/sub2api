<template>
  <BaseDialog :show="show" :title="t('admin.users.replaceGroupTitle')" width="narrow" @close="$emit('close')">
    <div v-if="oldGroup" class="space-y-4">
      ...
      <!-- 当前分组 -->
      <div class="border-y-2 border-accent py-2">
        <div class="flex items-center gap-2">
          <Icon name="shield" size="sm" class="text-fg-muted" />
          <span class="font-medium text-fg">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-fg-subtle" />
      ...
      <!-- 可选分组列表 -->
      <div v-if="availableGroups.length > 0" class="max-h-64 divide-y divide-border overflow-y-auto border-y border-border">
        <label
          v-for="group in availableGroups"
          :key="group.id"
          class="flex cursor-pointer items-center gap-3 px-2 py-2.5 transition-colors"
        ...
        >
          ...
          <div class="flex-1">
            ...
            <span class="ml-2 text-xs text-fg-subtle">{{ group.platform }}</span>
          </div>
        </label>
      </div>

      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-fg-subtle">
