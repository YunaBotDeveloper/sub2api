  ...
  >
    <div class="space-y-5">
      ...
      <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_240px]">
        <section class="min-h-[320px] max-h-[46vh] overflow-y-auto border border-border border-t-2 border-t-accent bg-surface p-5">
          <div class="legal-document-content" v-html="renderedDocument"></div>
        </section>

        <aside class="space-y-3 border-t border-border pt-3 text-body md:border-l md:border-t-0 md:pl-4 md:pt-0">
          <div>
            <p class="text-meta font-medium text-fg-muted">
              {{ t('adminCompliance.version') }}
            </p>
            <p class="mt-1 break-all font-mono text-fg">
          ...
          <a
            ...
            class="inline-flex items-center gap-2 text-accent underline underline-offset-4 hover:text-accent-strong"
          >
            <Icon name="externalLink" size="sm" />
            {{ t('adminCompliance.openDocument') }}
          </a>
          <p class="leading-6 text-fg-muted">
      ...
      <div class="space-y-3">
        <label for="admin-compliance-phrase" class="input-label mb-0">
          {{ t('adminCompliance.inputLabel') }}
        </label>
        <div class="code-block px-3 py-2">
          {{ expectedPhrase }}
        </div>
        <Input
      ...
      <p class="border-t border-border pt-3 text-meta leading-5 text-fg-muted">
...
.legal-document-content :deep(h1) {
  @apply mb-4 text-2xl font-bold text-fg;
}

.legal-document-content :deep(h2) {
  @apply mb-3 mt-6 text-xl font-semibold text-accent-strong;
}

.legal-document-content :deep(p) {
  @apply mb-4 text-sm text-fg-muted;
}

.legal-document-content :deep(ul),
.legal-document-content :deep(ol) {
  @apply mb-4 pl-6 text-sm text-fg-muted;
}
