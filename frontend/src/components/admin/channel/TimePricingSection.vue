  ...
  <section class="mt-3 border-t border-border pt-3">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div class="min-w-0 flex-1 sm:max-w-2xl">
        <label class="block text-label font-semibold text-fg">
      ...
      <button
        type="button"
        class="self-start text-xs text-accent hover:text-accent-strong sm:self-end sm:pb-2"
    ...
    <div v-if="modelValue.periods.length > 0" class="mt-3 space-y-3">
      <div
        ...
        :key="index"
        class="grid grid-cols-1 gap-2 border-t border-border pt-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_2rem] sm:items-end"
      >
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-start-${index}`" class="block text-xs text-fg-subtle">
        ...
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-end-${index}`" class="block text-xs text-fg-subtle">
        ...
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-multiplier-${index}`" class="block text-xs text-fg-subtle">
        ...
        <button
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded text-fg-subtle hover:text-danger"
