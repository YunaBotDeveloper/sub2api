<template>
  <div class="border border-accent/40 py-2 pl-3">
    ...
    >
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          <span
            ...
            :key="i"
            class="inline-flex shrink-0 rounded-sm px-1.5 py-0.5 font-mono text-xs"
          ...
          <span
            ...
            class="whitespace-nowrap text-xs text-fg-subtle"
          >
      ...
      <div v-else class="flex-1 text-label font-semibold text-accent-strong">
    ...
    >
      <div class="collapsible-inner">
        ...
        <div class="mt-3 flex items-start gap-2">
          <div class="flex-1">
            <label class="text-xs font-medium text-fg-muted">
              {{ t('admin.channels.form.models') }} <span class="text-danger">*</span>
            </label>
            <ModelTagInput
              :models="entry.models"
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-fg-muted">
            {{ t('admin.channels.form.defaultPrices') }}
            <span class="ml-1 font-normal text-fg-subtle">$/MTok</span>
          </label>
          <div class="pricing-default-grid mt-1 grid gap-2">
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.inputPrice') }}</label>
              <input :value="entry.input_price" @input="emitField('input_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.outputPrice') }}</label>
              <input :value="entry.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.cacheWrite5mPrice') }}</label>
              <input :value="entry.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.cacheWrite1hPrice') }}</label>
              <input :value="entry.cache_write_1h_price" @input="emitField('cache_write_1h_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.cacheReadPrice') }}</label>
              <input :value="entry.cache_read_price" @input="emitField('cache_read_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.imageInputPrice') }}</label>
              <input :value="entry.image_input_price" @input="emitField('image_input_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.imageTokenPrice') }}</label>
          ...
          <div v-if="enableTierMultipliers" class="mt-3 grid max-w-md grid-cols-1 gap-2 sm:grid-cols-2">
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.fastMultiplier') }}</label>
              <input :value="entry.fast_multiplier" @input="emitField('fast_multiplier', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0.000001" class="input mt-0.5 text-sm tabular-nums" :placeholder="t('admin.channels.form.multiplierPlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-fg-subtle">{{ t('admin.channels.form.flexMultiplier') }}</label>
          ...
          <div v-if="!hideTokenIntervals" class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-fg-muted">
                ...
                <span class="ml-1 font-normal text-fg-subtle">(min, max]</span>
              </label>
              <button type="button" @click="addInterval" class="text-xs text-accent hover:text-accent-strong">
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-fg-muted">
            ...
            <span class="ml-1 font-normal text-fg-subtle">$</span>
          </label>
          <div class="mt-1 w-48">
          ...
          <div v-else class="mt-2 border-y border-border py-2 text-center text-xs text-fg-subtle">
        ...
        <div v-else-if="entry.billing_mode === 'image' || entry.billing_mode === 'video'">
          ...
          <div class="mt-3 flex items-center justify-between">
            ...
            <button type="button" @click="addMediaTier" class="text-xs text-accent hover:text-accent-strong">
              + {{ t('admin.channels.form.addTier') }}
            </button>
          </div>
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
        ...
        <div class="mt-3 border-t border-border pt-3" data-testid="reasoning-effort-multipliers">
          <div class="flex items-center justify-between gap-2">
            <label class="text-xs font-medium text-fg-subtle">
          ...
          <p class="mt-1 text-xs text-fg-subtle">{{ t('admin.channels.form.reasoningEffortMultipliersHint') }}</p>
          <div class="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
            <label v-for="effort in REASONING_EFFORT_LEVELS" :key="effort" class="text-xs text-fg-subtle">
              {{ effort }}
              <input
                ...
                step="any"
                min="0"
                class="input mt-0.5 text-sm tabular-nums"
                :placeholder="t('admin.channels.form.reasoningEffortMultiplierDefault')"
              />
            </label>
          </div>
          <p v-if="reasoningEffortMultiplierError" role="alert" class="mt-1 text-xs text-danger">
