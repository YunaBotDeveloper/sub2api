      ...
      >
        <template #cell-user="{ row }">
          <div class="text-sm">
            ...
            <span v-if="row.user?.deleted_at" class="ml-1 inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-danger-100 text-danger-600 ring-1 ring-inset ring-danger-200 dark:bg-danger-500/20 dark:text-danger-400 dark:ring-danger-500/30">
        ...
        <template #cell-model="{ row }">
          <div class="space-y-0.5 text-xs">
            ...
            <div
              ...
              :class="isLikelyModelVariant(row) ? 'text-warning-600 dark:text-warning-400' : 'text-warning-600 dark:text-warning-400'"
        ...
        <template #cell-group="{ row }">
          <span v-if="row.group" class="inline-flex items-center rounded px-2 py-0.5 text-xs font-medium bg-accent-100 text-accent-800 dark:bg-accent-900 dark:text-accent-200">
            {{ row.group.name }}
          </span>
        ...
        <template #cell-stream="{ row }">
          <div class="flex flex-wrap items-center gap-1">
            ...
            <span
              ...
              class="inline-flex items-center rounded bg-accent-100 px-2 py-0.5 text-xs font-medium text-accent-800 dark:bg-accent-900 dark:text-accent-200"
            >
        ...
        <template #cell-tokens="{ row }">
          ...
          <div v-else class="flex items-center gap-1.5">
            <div class="space-y-1 text-sm">
              ...
              <div v-if="row.cache_read_tokens > 0 || row.cache_creation_tokens > 0" class="flex items-center gap-2">
                <div v-if="row.cache_read_tokens > 0" class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-accent-600 dark:text-accent-400">{{ formatCacheTokens(row.cache_read_tokens) }}</span>
                </div>
                <div v-if="row.cache_creation_tokens > 0" class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-warning-600 dark:text-warning-400">{{ formatCacheTokens(row.cache_creation_tokens) }}</span>
                  <span v-if="row.cache_creation_1h_tokens > 0" class="inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-warning-100 text-warning-600 ring-1 ring-inset ring-warning-200 dark:bg-warning-500/20 dark:text-warning-400 dark:ring-warning-500/30">1h</span>
                  <span v-if="row.cache_ttl_overridden" :title="t('usage.cacheTtlOverriddenHint')" class="inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-danger-100 text-danger-600 ring-1 ring-inset ring-danger-200 dark:bg-danger-500/20 dark:text-danger-400 dark:ring-danger-500/30 cursor-help">R</span>
                </div>
              </div>
              <div v-if="hasImageInputTokens(row)" class="flex items-center gap-2">
                <div class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-gray-600 dark:text-gray-400">{{ row.image_input_tokens.toLocaleString() }}</span>
                </div>
              </div>
              <div v-if="hasImageOutputTokens(row)" class="flex items-center gap-2">
                <div class="inline-flex items-center gap-1">
                  <svg class="h-3.5 w-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  <span class="font-medium text-gray-600 dark:text-gray-400">{{ row.image_output_tokens.toLocaleString() }}</span>
                </div>
              </div>
            </div>
            ...
            >
              <div class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-gray-100 transition-colors group-hover:bg-accent-100 dark:bg-gray-700 dark:group-hover:bg-accent-900/50">
                <Icon name="infoCircle" size="xs" class="text-gray-400 group-hover:text-accent-500 dark:text-gray-500 dark:group-hover:text-accent-400" />
              </div>
            </div>
          </div>
        </template>
        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-success-600 dark:text-success-400">${{ row.actual_cost?.toFixed(6) || '0.000000' }}</span>
              <span
                v-if="row.long_context_billing_applied"
                data-testid="long-context-billing-marker"
                class="inline-flex items-center rounded px-1 py-px text-[10px] font-semibold leading-tight bg-warning-100 text-warning-700 ring-1 ring-inset ring-warning-200 dark:bg-warning-500/20 dark:text-warning-300 dark:ring-warning-500/30"
              >x2</span>
              <!-- Cost Detail Tooltip -->
              <div
                class="group relative"
            ...
            <div v-if="showAccountBilling && row.account_rate_multiplier != null" class="mt-0.5 text-[11px] text-warning-500 dark:text-warning-400">
        ...
        <div class="space-y-1.5">
          <div>
            ...
            <div v-if="tokenTooltipData && hasImageInputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-gray-300">{{ tokenTooltipData.image_input_tokens.toLocaleString() }}</span>
            </div>
            ...
            <div v-if="tokenTooltipData && hasImageOutputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-gray-300">{{ tokenTooltipData.image_output_tokens.toLocaleString() }}</span>
            </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && hasImageInputCost(tooltipData)" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-gray-300">${{ tooltipData.image_input_cost.toFixed(8) }}</span>
            </div>
            ...
            <div v-if="tooltipData && hasImageOutputCost(tooltipData)" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-gray-300">${{ tooltipData.image_output_cost.toFixed(8) }}</span>
            </div>
            ...
            <template v-if="tooltipData && !isImageUsage(tooltipData) && (!tooltipData.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN)">
              ...
              <div v-if="tooltipData && hasImageInputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-gray-300">{{ formatTokenPricePerMillion(tooltipData.image_input_cost ?? 0, tooltipData.image_input_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && tooltipData.output_cost > 0 && textOutputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-gray-300">{{ formatTokenPricePerMillion(tooltipData.output_cost, textOutputTokens(tooltipData)) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && hasImageOutputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-gray-300">{{ formatTokenPricePerMillion(tooltipData.image_output_cost ?? 0, tooltipData.image_output_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
            </template>
            <template v-else-if="tooltipData && isImageUsage(tooltipData)">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-accent-300">${{ imageUnitPrice(tooltipData).toFixed(8) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
            ...
            <div v-else class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-accent-300">${{ tooltipData?.total_cost?.toFixed(8) || '0.00000000' }}</span>
            </div>
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-success-400">${{ tooltipData?.actual_cost?.toFixed(8) || '0.00000000' }}</span>
          </div>
...
const getRequestTypeBadgeClass = (row: AdminUsageLog): string => {
  const requestType = resolveUsageRequestType(row)
  if (requestType === 'cyber') return 'bg-danger-100 text-danger-800 dark:bg-danger-900 dark:text-danger-200'
  if (requestType === 'live') return 'bg-success-100 text-success-800 dark:bg-success-900 dark:text-success-200'
  if (requestType === 'ws_v2') return 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-200'
