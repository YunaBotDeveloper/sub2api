  ...
  >
    <div class="space-y-3">
      ...
      <div class="relative">
        ...
        />
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-fg-subtle" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
        </svg>
      </div>

      <div v-if="loading" class="py-6 text-center text-sm text-fg-muted">
        {{ t('common.loading') }}
      </div>
      <div v-else-if="filteredKeys.length === 0" class="py-6 text-center text-sm text-fg-muted">
        ...
        <table class="table">
          ...
          <tbody>
            <tr
              ...
              @click="$emit('pick', k)"
            >
              <td class="font-medium text-fg">{{ k.name }}</td>
              <td class="font-mono text-xs text-fg-muted">{{ maskApiKey(k.key) }}</td>
              <td>
                <GroupBadge
                  v-if="k.group"
                  :name="k.group.name"
