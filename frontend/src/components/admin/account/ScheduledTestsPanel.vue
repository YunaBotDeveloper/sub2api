<template>
  ...
  >
    <div class="space-y-4">
      ...
      >
        <div class="mb-3 text-sm font-medium text-fg">
          {{ t('admin.scheduledTests.addPlan') }}
        </div>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div>
            <label class="mb-1 flex items-center gap-1 text-xs font-medium text-fg-muted">
              {{ t('admin.scheduledTests.cronExpression') }}
              <HelpTooltip>
                <template #trigger>
                  <span class="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full border border-border-strong text-[10px] font-semibold text-fg-subtle transition-colors hover:border-accent hover:text-accent">
                    ?
                  </span>
                </template>
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="md" class="animate-spin text-fg-subtle" :stroke-width="2" />
        <span class="ml-2 text-sm text-fg-muted">{{ t('common.loading') }}...</span>
      </div>

      <!-- Empty State -->
      <div
        v-else-if="plans.length === 0"
        class="rounded-sm border border-dashed border-border-strong py-10 text-center"
      >
        <Icon name="calendar" size="lg" class="mx-auto mb-2 text-fg-subtle" :stroke-width="1.5" />
        <p class="text-sm text-fg-muted">
      ...
      <div v-else class="space-y-3">
        <div
          v-for="plan in plans"
          :key="plan.id"
          class="rounded-sm border border-border bg-surface transition-all"
        >
          <!-- Plan Header -->
          <div
          ...
          >
            ...
            <div class="flex items-center gap-3">
              <!-- Last Run -->
              <div v-if="plan.last_run_at" class="hidden text-right text-xs text-fg-muted sm:block">
                <div>{{ t('admin.scheduledTests.lastRun') }}</div>
                <div>{{ formatDateTime(plan.last_run_at) }}</div>
              </div>

              <!-- Next Run -->
              <div v-if="plan.next_run_at" class="hidden text-right text-xs text-fg-muted sm:block">
              ...
              <div class="flex items-center gap-1" @click.stop>
                <button
                  ...
                  class="rounded-sm p-1.5 text-fg-subtle transition-colors hover:bg-accent-weak hover:text-accent"
                ...
                <button
                  ...
                  class="rounded-sm p-1.5 text-fg-subtle transition-colors hover:bg-danger-weak hover:text-danger"
          ...
          <div
            ...
            @click.stop
          >
            <div class="mb-2 text-xs font-medium text-fg-muted">
          ...
          >
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              <Icon name="refresh" size="sm" class="animate-spin text-fg-subtle" :stroke-width="2" />
              <span class="ml-2 text-xs text-fg-muted">{{ t('common.loading') }}...</span>
            </div>

            <!-- No Results -->
            <div
              v-else-if="results.length === 0"
              class="py-4 text-center text-xs text-fg-muted"
            >
              ...
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <!-- Latency -->
                    <span v-if="result.latency_ms > 0" class="text-xs text-fg-muted">
                ...
                <div v-if="result.error_message" class="mt-2">
                  ...
                  <pre
                    ...
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded-sm bg-danger-weak p-2 text-xs text-danger-strong"
                  >{{ result.error_message }}</pre>
                </div>
                <div v-else-if="result.response_text" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-fg-muted"
                    @click="toggleResultDetail(result.id)"
                  >
                  ...
                  <pre
                    v-if="expandedResultIds.has(result.id)"
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded-sm bg-surface-sunken p-2 text-xs text-fg"
