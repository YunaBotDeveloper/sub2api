<template>
  <section class="mt-5 border-t border-border pt-5" aria-labelledby="backup-archive-title">
    <div class="mb-3 flex items-center justify-between gap-3">
      <h4 id="backup-archive-title" class="text-sm font-medium text-fg">{{ t('admin.backup.archive.title') }}</h4>
    ...
    <div v-if="modelValue.enabled">
      <div class="grid grid-cols-1 items-start gap-3 md:grid-cols-2">
        <div>
          <label for="backup-archive-dates" class="input-label">{{ t('admin.backup.archive.dates') }}</label>
          <div ref="datePicker" class="relative" @keydown.esc.stop.prevent="closeDates(true)" @focusout="handleFocusOut">
            ...
            >
              <span class="flex min-w-0 flex-wrap items-center gap-1">
                <span v-if="!selectedLabels.length" class="text-fg-subtle">{{ t('admin.backup.archive.selectDates') }}</span>
                <span v-for="label in selectedLabels.slice(0, 3)" :key="label" class="badge badge-primary">{{ label }}</span>
                <span v-if="selectedLabels.length > 3" class="text-xs">+{{ selectedLabels.length - 3 }}</span>
              </span>
              <Icon name="chevronDown" size="sm" class="shrink-0" />
            </button>
            <div v-if="datesOpen" id="backup-archive-options" class="absolute left-0 top-full z-30 mt-1 w-full border border-border bg-surface-raised p-2 shadow-lg">
              <div class="mb-1 flex items-center justify-between gap-2 px-1 text-xs text-fg-muted">
                <span>{{ t('admin.backup.archive.selectedDates', { count: selectedLabels.length }) }}</span>
                <button type="button" class="min-h-9 px-2 text-accent-strong" @click="closeDates(true)">{{ t('admin.backup.archive.done') }}</button>
              </div>
              <div class="grid grid-cols-4 gap-1" role="group" :aria-label="t('admin.backup.archive.dates')">
                <label v-for="day in 31" :key="day" class="flex min-h-10 cursor-pointer items-center gap-1.5 rounded px-1 text-xs hover:bg-surface-sunken">
                ...
                <label class="flex min-h-10 cursor-pointer items-center gap-1.5 rounded px-1 text-xs hover:bg-surface-sunken">
          ...
          <p id="backup-archive-dates-hint" class="input-hint">{{ t('admin.backup.archive.datesHint') }}</p>
        </div>
        <div>
          <span class="input-label">{{ t('admin.backup.archive.retention') }}</span>
          <div class="flex min-h-10 flex-wrap items-center gap-3">
            ...
            <label class="inline-flex min-h-10 shrink-0 items-center gap-2 text-sm text-fg">
              <input data-testid="archive-forever" type="checkbox" :checked="forever" @change="toggleForever" />
              {{ t('admin.backup.archive.forever') }}
            </label>
          </div>
          <p id="backup-archive-retention-hint" class="input-hint">{{ t(forever ? 'admin.backup.archive.foreverHint' : 'admin.backup.archive.countHint') }}</p>
        </div>
      </div>
      <p class="mt-3 text-xs text-fg-muted">{{ t('admin.backup.archive.fallbackHint') }}</p>
      <p class="input-hint">{{ t('admin.backup.archive.independentHint') }}</p>
    </div>
    <p v-else class="text-xs text-fg-muted">{{ t('admin.backup.archive.disabledHint') }}</p>
  </section>
</template>
