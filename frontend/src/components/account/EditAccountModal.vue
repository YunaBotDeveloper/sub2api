    ...
    >
          ...
          <div class="mt-2 grid grid-cols-1 border-l border-t border-border sm:grid-cols-2">
            ...
            >
              <div
                :class="[
                  ...
                  editOpenCodeAccountMode === 'zen' ? 'text-accent' : 'text-fg-subtle'
                ]"
              >
            ...
            >
              <div
                :class="[
                  'flex h-4 w-4 shrink-0 items-center justify-center',
                  editOpenCodeAccountMode === 'go' ? 'text-accent' : 'text-fg-subtle'
                ]"
              >
        ...
        <div v-if="isCNApiKeyAccount && account.platform !== 'opencode_go'">
          ...
          <div class="mt-2 flex flex-wrap gap-2">
            <button
              ...
              type="button"
              :class="[
                'rounded-sm border-2 px-3 py-1.5 text-xs transition-all',
      ...
      >
        <div class="space-y-3">
          ...
          <p v-if="grokMediaEligibilityLoading" class="text-xs text-fg-muted">
            {{ t('admin.accounts.grokMediaEligibility.loading') }}
          </p>
          <p v-else-if="grokMediaEligibilityError" class="text-xs text-danger">
            {{ grokMediaEligibilityError }}
          </p>
          <div v-else-if="grokMediaEligibilityState" class="rounded-sm bg-surface-sunken p-3 text-xs">
          ...
          <div
            ...
            class="border border-warning/40 bg-warning-weak px-3 py-2"
          >
            <p class="text-xs text-warning-strong">
              <Icon name="exclamationTriangle" size="sm" class="mr-1 inline" :stroke-width="2" />
          ...
          <p v-else-if="grokMediaEligibilityMode === 'auto'" class="text-xs text-fg-muted">
      ...
      <div v-if="headerOverrideCapable" class="border-t border-border pt-4">
        <div class="mb-3 flex items-center justify-between">
          <div>
      ...
      <div v-if="account.platform === 'antigravity'" class="border-t border-border pt-4">
        ...
        <div>
          <div class="mb-3 border-t border-border pt-3">
            <p class="text-xs text-fg">{{ t('admin.accounts.mapRequestModels') }}</p>
          </div>
      ...
      >
        <div>
          ...
          <div class="mt-3">
            <div class="grid grid-cols-1 border-l border-t border-border sm:grid-cols-2">
              <button
                ...
                :class="[
                  'group flex min-h-[62px] items-start gap-2 border-b border-r border-border px-3 py-2 text-left transition-colors',
              ...
              >
                ...
                <span class="min-w-0">
                  ...
                  <span class="mt-0.5 block text-xs leading-4 text-fg-muted">{{ option.description }}</span>
                </span>
              </button>
            </div>
          </div>
      ...
      >
        ...
        <div
          ...
          class="rounded-sm bg-surface-sunken px-3 py-2 text-xs text-fg-muted"
        >
        ...
        <template v-else>
          ...
          >
            ...
            <p
              ...
              class="mt-2 break-words border-t border-border pt-2 text-xs text-warning-strong"
            >
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2',
                autoPause5hDisabled ? 'bg-accent' : 'bg-border'
              ]"
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2',
                autoPause7dDisabled ? 'bg-accent' : 'bg-border'
              ]"
      ...
      >
        <div class="flex items-center justify-between gap-4">
          ...
          <button
            ...
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2',
              autoResetCreditEnabled ? 'bg-accent' : 'bg-border'
            ]"
          >
            <span
              :class="[
      ...
      >
        <div class="min-w-0">
          <label class="input-label mb-0">{{ t('admin.accounts.codexCredits.title') }}</label>
          <p class="mt-1 text-xs text-fg-muted">{{ t('admin.accounts.codexCredits.hint') }}</p>
        </div>
        <button
          type="button"
          data-testid="codex-credits-enabled"
          @click="codexCreditsEnabled = !codexCreditsEnabled"
          :class="[
            'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2',
            codexCreditsEnabled ? 'bg-accent' : 'bg-border'
          ]"
        >
          <span
            :class="[
              'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-surface ring-0 transition duration-200 ease-in-out',
              codexCreditsEnabled ? 'translate-x-5' : 'translate-x-0'
            ]"
          />
        </button>
      </div>
        ...
        <div v-if="account?.platform === 'antigravity'" class="flex items-center gap-2">
          <label class="flex cursor-not-allowed items-center gap-2 opacity-60">
            <input
              ...
              disabled
              class="h-4 w-4 cursor-not-allowed rounded-sm border-border-strong text-accent focus:ring-accent"
            />
...
const codexCreditsEnabled = ref(false)
...
const codexImageToolOptions = computed<Array<{
...
}>>(() => [
  {
    ...
    selectedCardClass: 'bg-accent-weak text-accent-strong shadow-[inset_3px_0_0_rgb(var(--accent))]',
  ...
  },
  {
    ...
    selectedCardClass: 'bg-success-weak text-success-strong shadow-[inset_3px_0_0_rgb(var(--success))]',
  ...
  },
  {
    ...
    selectedCardClass: 'bg-warning-weak text-warning-strong shadow-[inset_3px_0_0_rgb(var(--warning))]',
  ...
  },
  {
    ...
    selectedCardClass: 'bg-danger-weak text-danger-strong shadow-[inset_3px_0_0_rgb(var(--danger))]',
    selectedDotClass: 'border-danger bg-danger text-white'
  }
])
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  allowOverages.value = false
	...
	codexCreditsEnabled.value = extra?.codex_credits_enabled === true
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      if (openAICompactMode.value === 'auto') {
		...
		if (props.account.type === 'oauth' && !isSparkShadow.value) {
			...
			if (codexCreditsEnabled.value) {
				newExtra.codex_credits_enabled = true
			} else {
				delete newExtra.codex_credits_enabled
			}
