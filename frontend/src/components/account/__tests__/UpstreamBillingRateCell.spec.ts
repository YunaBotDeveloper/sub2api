...
describe('UpstreamBillingRateCell', () => {
  ...
  it('shows stale snapshot details, local next probe time, and the account probe state', async () => {
    ...
    expect(tooltip.querySelector('[data-testid="upstream-billing-probe-state"] span')?.className).toContain('text-success')
    ...
    expect(tooltip.querySelector('[data-testid="upstream-billing-probe-state"] span')?.className).toContain('text-danger')
    wrapper.unmount()
  })
  ...
  it('stacks the global-off state below the account state and hides it when globally enabled', async () => {
    ...
    expect(accountState?.querySelector('span')?.className).toContain('text-success')
    expect(globalState?.textContent).toContain('admin.accounts.upstreamBilling.globalProbeState')
    expect(globalState?.querySelector('span')?.className).toContain('text-danger')
    expect(tooltip.querySelector('[data-testid="upstream-billing-next-probe"]')).toBeNull()
    ...
    expect(accountState?.querySelector('span')?.className).toContain('text-danger')
