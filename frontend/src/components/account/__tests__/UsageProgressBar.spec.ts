...
describe('UsageProgressBar', () => {
  ...
  it('默认利用率模式按 75/90 阈值提前预警分级', () => {
    ...
    expect(mountAt(74).get('.h-1\\.5 > div').classes()).toContain('bg-success')
    expect(mountAt(75).get('.h-1\\.5 > div').classes()).toContain('bg-warning')
    expect(mountAt(89).get('.h-1\\.5 > div').classes()).toContain('bg-warning')
    expect(mountAt(90).get('.h-1\\.5 > div').classes()).toContain('bg-danger')

    // 百分比文本同步分级
    expect(mountAt(74).get('.h-1\\.5 + span').classes()).toContain('text-fg-muted')
    expect(mountAt(75).get('.h-1\\.5 + span').classes()).toContain('text-warning')
    expect(mountAt(89).get('.h-1\\.5 + span').classes()).toContain('text-warning')
    expect(mountAt(90).get('.h-1\\.5 + span').classes()).toContain('text-danger')
  })
