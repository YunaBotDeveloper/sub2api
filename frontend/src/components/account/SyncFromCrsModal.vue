<template>
  ...
  >
    ...
    >
      ...
      <div
        class="rounded-sm bg-surface-sunken p-3 text-xs text-fg-muted"
      >
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <label class="flex items-center gap-2 text-sm text-fg">
          <input
            ...
            type="checkbox"
            class="rounded-sm border-border-strong"
          />
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      >
        <div class="mb-2 text-sm font-medium text-fg">
          {{ t('admin.accounts.crsExistingAccounts') }}
          <span class="ml-1 text-xs text-fg-subtle">({{ previewResult.existing_accounts.length }})</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-fg-muted">
          <div
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-fg">
            ...
            <span class="ml-1 text-xs text-fg-subtle">({{ previewResult.new_accounts.length }})</span>
          </div>
          <div class="flex gap-2">
            ...
            <button
              type="button"
              class="text-xs text-fg-muted hover:text-fg-muted"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') }}</button>
          </div>
        </div>
        <div
          class="max-h-48 overflow-auto rounded-sm border border-border p-2"
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1.5 hover:bg-surface-sunken"
          >
            <input
              type="checkbox"
            ...
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-fg">{{ acc.name }}</span>
          </label>
        </div>
      ...
      <div class="flex items-center gap-2 text-xs text-fg-muted">
        ...
        <span :class="form.sync_proxies ? 'text-success' : 'text-fg-subtle'">
      ...
      <div
        ...
        class="rounded-sm bg-surface-sunken p-4 text-center text-sm text-fg-muted"
      >
