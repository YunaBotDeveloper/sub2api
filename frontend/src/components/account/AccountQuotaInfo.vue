    ...
    <div class="text-xs text-gray-400 dark:text-gray-500">
      ...
      <span
        ...
        :class="[
          'font-medium',
          isUrgent
            ? 'text-danger-600 dark:text-danger-400 animate-pulse'
...
const tierBadgeClass = computed(() => {
  ...
  if (isCodeAssist.value) {
    const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
    if (tier === 'gcp_enterprise') return 'bg-gray-100 text-gray-600 dark:bg-gray-900/40 dark:text-gray-300'
    if (tier === 'gcp_standard') return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
    if (upper.includes('ULTRA') || upper.includes('ENTERPRISE')) return 'bg-gray-100 text-gray-600 dark:bg-gray-900/40 dark:text-gray-300'
    return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
  }

  if (isGoogleOne.value) {
    const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
    if (tier === 'google_ai_ultra') return 'bg-gray-100 text-gray-600 dark:bg-gray-900/40 dark:text-gray-300'
    if (tier === 'google_ai_pro') return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
    if (tier === 'google_one_free') return 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
    if (upper === 'GOOGLE_ONE_UNLIMITED') return 'bg-gray-100 text-gray-600 dark:bg-gray-900/40 dark:text-gray-300'
    if (upper === 'AI_PREMIUM') return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
    return 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'
  }

  // AI Studio 默认样式：蓝色
  const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
  if (tier === 'aistudio_paid') return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
