<template>
  <div class="overflow-x-auto">
    <table class="table min-w-[1000px] table-auto tabular-nums">
      <colgroup>
        <col class="w-[25%]" />
        <col class="w-[11%]" />
        <col class="w-[9%]" />
      ...
      <thead>
        <tr>
          ...
          <th colspan="3" class="text-center">
            ...
            <span class="ml-1 font-normal text-fg-muted">{{ t('modelPlaza.table.unitPerMillion') }}</span>
          </th>
          <th colspan="3" class="text-center">
        ...
        <tr>
          <th class="text-right">{{ t('modelPlaza.table.input') }}</th>
          <th class="text-right">{{ t('modelPlaza.table.output') }}</th>
          <th>{{ t('modelPlaza.table.cache') }}</th>
          <th class="text-right font-medium text-fg-muted">{{ t('modelPlaza.table.input') }}</th>
          <th class="text-right font-medium text-fg-muted">{{ t('modelPlaza.table.output') }}</th>
          <th class="font-medium text-fg-muted">{{ t('modelPlaza.table.cache') }}</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="{ model: m, period, key } in rows" :key="key">
          <!-- 模型名 + 非 token 计费模式徽章;分时时段行额外标注时段 -->
          <td class="align-middle">
            <div class="flex flex-wrap items-center gap-1.5">
              <span class="font-mono text-label font-medium text-fg">{{ m.name }}</span>
          ...
          <template v-if="billingMode(m) === BILLING_MODE_TOKEN">
            <td class="text-right align-middle font-semibold text-fg">
              <template v-if="tokenIntervals(m).length">
                <div
                  v-for="(iv, idx) in tokenIntervals(m)"
                  :key="idx"
                  class="whitespace-nowrap text-label leading-5"
                >
                  <span class="mr-1 font-normal text-fg-subtle" :title="tierHint(m)">{{ tierLabel(iv) }}</span>
            ...
            <td class="align-middle">
              <template v-if="hasTierCachePricing(tokenIntervals(m))">
                <div
                  v-for="(iv, idx) in tokenIntervals(m)"
                  :key="idx"
                  class="whitespace-nowrap text-label leading-5 text-fg"
                  :title="tierHint(m)"
                >
                  <template v-if="iv.cache_write_price != null || iv.cache_write_1h_price != null || iv.cache_read_price != null">
                    <span class="font-normal text-fg-subtle">{{ t('modelPlaza.table.cacheWriteShort') }}</span>
                    {{ paidPerMillion(iv.cache_write_price, period) }}
                    <template v-if="iv.cache_write_1h_price != null"
                      ><span class="font-normal text-fg-subtle"> (1h </span>{{ paidPerMillion(iv.cache_write_1h_price, period)
                      }}<span class="font-normal text-fg-subtle">)</span></template
                    >
                    <span class="ml-1 font-normal text-fg-subtle">{{ t('modelPlaza.table.cacheReadShort') }}</span>
              ...
              <div
                ...
                class="space-y-0.5 text-label text-fg"
              >
                <div>
                  <span class="mr-1 font-normal text-fg-subtle">{{ t('modelPlaza.table.cacheWrite') }}</span>
                  {{ paidPerMillion(m.pricing?.cache_write_price, period)
                  }}<template v-if="m.pricing?.cache_write_1h_price != null"
                    ><span class="font-normal text-fg-subtle"> (1h </span>{{ paidPerMillion(m.pricing.cache_write_1h_price, period)
                    }}<span class="font-normal text-fg-subtle">)</span></template
                  >
                </div>
                <div>
                  <span class="mr-1 font-normal text-fg-subtle">{{ t('modelPlaza.table.cacheRead') }}</span>
            ...
            <td colspan="3" class="align-middle">
              ...
              >
                <span
                  ...
                  :key="idx"
                  class="badge badge-gray text-fg"
                >
                  <span class="text-fg-subtle">{{ tierLabel(iv) }}</span>
                  {{ paidRequestPrice(m, iv.per_request_price)
                  }}<span class="text-fg-subtle">{{ perUnitSuffix(m) }}</span>
                </span>
              </div>
              <template v-else-if="m.pricing?.per_request_price != null">
                ...
                <span class="ml-1 text-meta text-fg-subtle">{{ perUnitSuffix(m) }}</span>
              </template>
              <span v-else class="text-fg-subtle">-</span>
            </td>
          </template>
          ...
          <td
            class="text-right align-middle text-label text-fg-muted"
          >
            <template v-if="officialIntervals(m).length">
              ...
              >
                <span class="mr-1 text-fg-subtle" :title="t('modelPlaza.table.tierHint')">{{ tierLabel(iv) }}</span>
                {{ official(iv.input_price) }}
              </div>
            </template>
            <template v-else>{{ official(m.official_pricing?.input_price) }}</template>
          </td>
          <td class="text-right align-middle text-label text-fg-muted">
            <template v-if="officialIntervals(m).length">
              <div
                v-for="(iv, idx) in officialIntervals(m)"
                :key="idx"
          ...
          <td class="align-middle">
            <template v-if="hasTierCachePricing(officialIntervals(m))">
              <div
                v-for="(iv, idx) in officialIntervals(m)"
                :key="idx"
                class="whitespace-nowrap text-label leading-5 text-fg-muted"
                :title="t('modelPlaza.table.tierHint')"
              >
                <template v-if="iv.cache_write_price != null || iv.cache_write_1h_price != null || iv.cache_read_price != null">
                  <span class="text-fg-subtle">{{ t('modelPlaza.table.cacheWriteShort') }}</span>
                  {{ official(iv.cache_write_price) }}
                  <template v-if="iv.cache_write_1h_price != null"
                    ><span class="text-fg-subtle"> (1h </span>{{ official(iv.cache_write_1h_price)
                    }}<span class="text-fg-subtle">)</span></template
                  >
                  <span class="ml-1 text-fg-subtle">{{ t('modelPlaza.table.cacheReadShort') }}</span>
            ...
            <div
              ...
              class="space-y-0.5 text-label text-fg-muted"
            >
              <div>
                <span class="mr-1 font-normal text-fg-subtle">{{ t('modelPlaza.table.cacheWrite') }}</span>
                {{ official(m.official_pricing.cache_write_price)
                }}<template v-if="m.official_pricing.cache_write_1h_price != null"
                  ><span class="text-fg-subtle"> (1h </span>{{ official(m.official_pricing.cache_write_1h_price)
                  }}<span class="text-fg-subtle">)</span></template
                >
              </div>
              <div>
          ...
          <td
            class="text-right align-middle text-label"
          >
            <span
              v-if="period"
              class="font-bold text-accent-strong"
            ...
            <template v-else-if="hasCustomRate">
              <span class="mr-1 text-fg-subtle line-through">{{ rateMultiplier }}x</span>
              <span class="font-bold text-accent-strong">{{ effectiveRate }}x</span>
            </template>
            <span v-else class="font-bold text-fg">{{ effectiveRate }}x</span>
          </td>
        </tr>
      </tbody>
    </table>
