  ...
  <AppLayout>
    <TablePageLayout>
      <template #filters>
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-sm border border-border bg-surface shadow-overlay"
                >
                  <div class="p-2">
                    <button
                      @click="setAutoRefreshEnabled(!autoRefreshEnabled)"
                      class="flex w-full items-center justify-between rounded-sm px-3 py-2 text-sm text-fg hover:bg-surface-sunken"
                    >
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                <Teleport to="body">
                  <div
                    ...
                    class="fixed z-[9999] origin-top-right overflow-hidden rounded-sm border border-border bg-surface shadow-overlay"
                    :style="accountToolsDropdownStyle"
                    @click.stop
                  >
                    <div class="overflow-y-auto p-2" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      <div class="px-2 py-2">
                        <div class="text-meta font-bold text-accent-strong">
                      ...
                      <div class="px-2 py-2">
                        <div class="flex items-center justify-between gap-3">
                          <span class="text-meta font-bold text-accent-strong">
                            {{ t('admin.accounts.viewColumns') }}
                          </span>
                          <Icon name="grid" size="sm" class="text-fg-subtle" />
                        </div>
                      </div>
                      <div class="grid grid-cols-1 gap-1">
                        <button
                          ...
                          class="flex w-full items-center justify-between rounded-sm px-3 py-2 text-sm text-fg transition-colors hover:bg-surface-sunken"
                        >
        ...
        <div
          ...
          class="mt-2 flex items-center justify-between border border-warning/40 bg-warning-weak px-3 py-2 text-sm text-warning-strong"
        >
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="isSelected(row.id)" @change="toggleSel(row.id)" class="rounded-sm border-border-strong text-accent focus:ring-accent" />
          </template>
          <template #cell-id="{ value }">
            <span class="font-mono text-xs text-fg-muted">#{{ value }}</span>
          </template>
              ...
              >
                <template #trigger>
                  <a
                    ...
                    class="border-b border-dotted border-border-strong font-medium text-fg"
                  >
                    {{ value }}
                  </a>
                </template>
              </HelpTooltip>
              <span v-else class="font-medium text-fg">{{ value }}</span>
              <span
                v-if="accountDisplayEmail(row)"
                class="text-xs text-fg-muted truncate max-w-[200px]"
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="block max-w-xs truncate text-sm text-fg-muted">{{ value }}</span>
            ...
            <div class="flex min-w-0 flex-col gap-1">
              <div class="flex flex-wrap items-center gap-1">
                ...
                <span
                  ...
                  :class="['badge', getAntigravityTierClass(row)]"
                >
          ...
          <template #cell-schedulable="{ row }">
            <button @click="handleToggleSchedulable(row)" :disabled="togglingSchedulable === row.id" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 focus:ring-offset-surface" :class="[row.schedulable ? 'bg-accent hover:bg-accent-strong' : 'bg-border hover:bg-border-strong']" :title="row.schedulable ? t('admin.accounts.schedulableEnabled') : t('admin.accounts.schedulableDisabled')">
              <span class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-surface ring-0 transition duration-200 ease-in-out" :class="[row.schedulable ? 'translate-x-4' : 'translate-x-0']" />
            </button>
          </template>
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              <div v-if="row.proxy" class="flex items-center gap-2">
                <span class="text-sm text-fg">{{ row.proxy.name }}</span>
                <span v-if="row.proxy.country_code" class="text-xs text-fg-muted">
                  ({{ row.proxy.country_code }})
                </span>
              </div>
              <span v-else class="text-sm text-fg-subtle">-</span>
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="text-fg-muted">{{ formatDateTime(row.proxy.expires_at) }}</span>
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                ...
                <button class="btn btn-secondary btn-sm px-1.5 py-0.5" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
              </div>
            </div>
          </template>
          <template #cell-rate_multiplier="{ row }">
            <span class="inline-flex items-center gap-1 text-sm tabular-nums text-fg">
              <span>{{ formatMultiplier(row.rate_multiplier ?? 1) }}x</span>
              <span
                v-if="row.extra?.upstream_billing_rate_sync_enabled === true"
                class="inline-flex cursor-help text-success"
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="flex min-w-[7rem] flex-col gap-0.5 text-[11px] leading-4 tabular-nums">
              <div
                v-for="score in getSchedulerScoreRows(row)"
                :key="String(score.group_id)"
                class="flex items-center gap-1 whitespace-nowrap text-fg"
                :title="`${formatSchedulerScoreGroup(score)} / ${formatSchedulerScore(score.base_score)} / ${formatStickySchedulerScore(score)}`"
              >
                <span class="max-w-[4.75rem] truncate text-fg-muted">{{ formatSchedulerScoreGroup(score) }}</span>
                <span class="text-fg-subtle">/</span>
                <span>{{ formatSchedulerScore(score.base_score) }}</span>
                <span class="text-fg-subtle">/</span>
                <span class="text-accent-strong">{{ formatStickySchedulerScore(score) }}</span>
              </div>
            </div>
            <span v-else class="text-sm text-fg-subtle">-</span>
          </template>
          <template #cell-last_used_at="{ value }">
            <span class="text-sm text-fg-muted">{{ formatRelativeTime(value) }}</span>
          </template>
          ...
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="text-sm text-fg-muted">{{ formatExpiresAt(value) }}</span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button @click="handleEdit(row)" class="flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-fg-muted transition-colors hover:bg-surface-sunken hover:text-accent">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                <span class="text-xs">{{ t('common.edit') }}</span>
              </button>
              <button @click="handleDelete(row)" class="flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-fg-muted transition-colors hover:bg-danger-weak hover:text-danger">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                <span class="text-xs">{{ t('common.delete') }}</span>
              </button>
              <button @click="openMenu(row, $event)" class="flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-fg-muted transition-colors hover:bg-surface-sunken hover:text-fg">
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-fg">
        <input type="checkbox" class="h-4 w-4 rounded-sm border-border-strong text-accent focus:ring-accent" v-model="includeProxyOnExport" />
...
function getAntigravityTierClass(row: any): string {
  const tier = getAntigravityTierFromRow(row)
  switch (tier) {
    case 'free-tier': return 'badge-gray'
    case 'g1-pro-tier': return 'badge-primary'
    case 'g1-ultra-tier': return 'badge-gray'
    default: return ''
  }
}
...
.account-tools-menu-item {
  @apply flex w-full items-center gap-3 rounded-sm px-3 py-2 text-sm text-fg transition-colors hover:bg-accent-weak hover:text-accent-strong;
}

.account-tools-menu-icon {
  @apply inline-flex h-4 w-4 flex-shrink-0 items-center justify-center text-accent;
}
</style>
