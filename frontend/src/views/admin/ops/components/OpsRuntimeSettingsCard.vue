...
<template>
  <div class="card p-5">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-body font-bold text-accent-strong">{{ t('admin.ops.runtime.title') }}</h3>
        <p class="mt-1 text-xs text-fg-muted">{{ t('admin.ops.runtime.description') }}</p>
      </div>
      <button
    ...
    <div v-if="!alertSettings" class="text-sm text-fg-muted">
    ...
    <div v-else class="space-y-6">
      <div class="border-t border-border pt-3">
        <div class="mb-3 flex items-center justify-between">
          <h4 class="text-body font-bold text-accent-strong">{{ t('admin.ops.runtime.alertTitle') }}</h4>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-fg-muted">
            ...
            <span class="ml-1 font-medium text-fg">{{ alertSettings.evaluation_interval_seconds }}s</span>
          </div>
          <div
            v-if="alertSettings.silencing?.enabled && alertSettings.silencing.global_until_rfc3339"
            class="text-xs text-fg-muted md:col-span-2"
          >
            {{ t('admin.ops.runtime.silencing.globalUntil') }}:
            <span class="ml-1 font-mono text-fg">{{ alertSettings.silencing.global_until_rfc3339 }}</span>
          </div>

          <details class="col-span-1 md:col-span-2">
            <summary class="cursor-pointer text-xs font-medium text-accent hover:text-accent-strong">
              {{ t('admin.ops.runtime.showAdvancedDeveloperSettings') }}
            </summary>
            <div class="mt-2 grid grid-cols-1 gap-3 border-l border-border pl-3 md:grid-cols-2">
              <div class="text-xs text-fg-muted">
                {{ t('admin.ops.runtime.lockEnabled') }}:
                <span class="ml-1 font-mono text-fg">{{ alertSettings.distributed_lock.enabled }}</span>
              </div>
              <div class="text-xs text-fg-muted">
                {{ t('admin.ops.runtime.lockKey') }}:
                <span class="ml-1 font-mono text-fg">{{ alertSettings.distributed_lock.key }}</span>
              </div>
              <div class="text-xs text-fg-muted">
                {{ t('admin.ops.runtime.lockTTLSeconds') }}:
                <span class="ml-1 font-mono text-fg">{{ alertSettings.distributed_lock.ttl_seconds }}s</span>
              </div>
            </div>
          </details>
        </div>
  ...
  <BaseDialog :show="showAlertEditor" :title="t('admin.ops.runtime.alertTitle')" width="extra-wide" @close="showAlertEditor = false">
    <div v-if="draftAlert" class="space-y-4">
      <div
        ...
        class="border border-warning/40 bg-warning-weak px-3 py-2 text-xs text-warning-strong"
      >
      ...
      <div>
        <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.evalIntervalSeconds') }}</div>
        <input
        ...
        />
        <p class="mt-1 text-xs text-fg-muted">{{ t('admin.ops.runtime.evalIntervalHint') }}</p>
      </div>

      <div class="border-t border-border pt-3">
        <div class="mb-2 text-body font-bold text-accent-strong">{{ t('admin.ops.runtime.metricThresholds') }}</div>
        <p class="mb-4 text-xs text-fg-muted">{{ t('admin.ops.runtime.metricThresholdsHint') }}</p>

        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.slaMinPercent') }}</div>
            <input
              ...
              placeholder="99.5"
            />
            <p class="mt-1 text-xs text-fg-muted">{{ t('admin.ops.runtime.slaMinPercentHint') }}</p>
          </div>



          <div>
            <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.ttftP99MaxMs') }}</div>
            <input
              ...
              placeholder="500"
            />
            <p class="mt-1 text-xs text-fg-muted">{{ t('admin.ops.runtime.ttftP99MaxMsHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.requestErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-fg-muted">{{ t('admin.ops.runtime.requestErrorRateMaxPercentHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-fg-muted">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercentHint') }}</p>
          </div>
        </div>
      </div>

      <div class="border-t border-border pt-3">
        <div class="mb-2 text-body font-bold text-accent-strong">{{ t('admin.ops.runtime.silencing.title') }}</div>

        <label class="inline-flex items-center gap-2 text-sm text-fg">
          <input v-model="draftAlert.silencing.enabled" type="checkbox" class="h-4 w-4 rounded-sm border-border-strong" />
          <span>{{ t('admin.ops.runtime.silencing.enabled') }}</span>
        </label>

        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          <div>
            <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.silencing.globalUntil') }}</div>
            <input
            ...
            />
            <p class="mt-1 text-xs text-fg-muted">{{ t('admin.ops.runtime.silencing.untilHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.silencing.reason') }}</div>
            <input
          ...
          <div class="border-t border-border pt-4">
            <div class="flex items-start justify-between gap-4">
              <div>
                <div class="text-xs font-bold text-fg">{{ t('admin.ops.runtime.silencing.entries.title') }}</div>
                <p class="text-[11px] text-fg-muted">{{ t('admin.ops.runtime.silencing.entries.hint') }}</p>
              </div>
            ...
            <div v-if="!draftAlert.silencing.entries?.length" class="mt-3 border-l border-border pl-3 text-xs text-fg-muted">
            ...
            <div v-else class="mt-4 space-y-4">
              ...
              >
                <div class="mb-3 flex items-center justify-between">
                  <div class="text-xs font-bold text-fg">
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.silencing.entries.ruleId') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.silencing.entries.severities') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.silencing.entries.until') }}</div>
                    <input
              ...
              placeholder="2026-01-05T00:00:00Z"
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.silencing.entries.reason') }}</div>
                    <input
      ...
      <details class="border-t border-border pt-3">
        <summary class="cursor-pointer text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.advancedSettingsSummary') }}</summary>
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="inline-flex items-center gap-2 text-xs text-fg">
              <input v-model="draftAlert.distributed_lock.enabled" type="checkbox" class="h-4 w-4 rounded-sm border-border-strong" />
              <span>{{ t('admin.ops.runtime.lockEnabled') }}</span>
            </label>
          </div>
          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.lockKey') }}</div>
            <input v-model="draftAlert.distributed_lock.key" type="text" class="input text-xs font-mono" />
            <p v-if="draftAlert.distributed_lock.enabled" class="mt-1 text-[11px] text-fg-muted">
              {{ t('admin.ops.runtime.validation.lockKeyHint', { prefix: 'ops:' }) }}
            </p>
          </div>
          <div>
            <div class="mb-1 text-xs font-medium text-fg-muted">{{ t('admin.ops.runtime.lockTTLSeconds') }}</div>
