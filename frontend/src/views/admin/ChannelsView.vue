      ...
      <template #table>
        ...
        >
          ...
          <template #cell-description="{ value }">
            <span class="text-sm text-fg-muted">{{ value || '-' }}</span>
          </template>
          ...
          <template #cell-group_count="{ row }">
            <span class="whitespace-nowrap text-sm tabular-nums">
              <span class="font-semibold text-fg">{{ (row.group_ids || []).length }}</span>
              <span class="text-fg-subtle">{{ t('admin.channels.groupsUnit', 'groups') }}</span>
            </span>
          </template>

          <template #cell-pricing_count="{ row }">
            <span class="whitespace-nowrap text-sm tabular-nums">
              <span class="font-semibold text-fg">{{ (row.model_pricing || []).length }}</span>
              <span class="text-fg-subtle">{{ t('admin.channels.pricingUnit', 'pricing rules') }}</span>
            </span>
          </template>
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <div>
              ...
              <p class="mt-1 ml-6 text-xs text-fg-subtle">
            ...
            <div class="space-y-3">
              ...
              <div class="flex flex-wrap gap-2">
                <label
                  ...
                  :key="p"
                  class="inline-flex cursor-pointer items-center gap-1.5 rounded-sm border px-3 py-1.5 text-sm transition-colors"
                  :class="activePlatforms.includes(p)
                    ? 'border-accent bg-accent-weak'
                    : 'border-border-strong hover:bg-accent-weak/50'"
                >
                  <input
                    type="checkbox"
            ...
            <div class="border-t border-border pt-4">
              <div class="flex items-center justify-between">
                <div>
                  <label class="text-label font-semibold text-fg">
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                ...
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-fg-subtle">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }) }})
                </span>
              </label>
              <div class="max-h-40 overflow-auto border border-border p-2">
                <div v-if="groupsLoading" class="py-2 text-center text-xs text-fg-muted">
                  {{ t('common.loading', 'Loading...') }}
                </div>
                <div v-else-if="getGroupsForPlatform(section.platform).length === 0" class="py-2 text-center text-xs text-fg-muted">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-sm border border-border px-2 py-1 text-xs transition-colors hover:bg-accent-weak/50"
                    :class="[
                      section.group_ids.includes(group.id) ? 'border-accent bg-accent-weak' : '',
                  ...
                  >
                    <input
                      ...
                      class="h-3 w-3 rounded border-border-strong text-accent focus:ring-accent"
                      @change="toggleGroupInSection(sIdx, group.id)"
                    />
                    <span :class="['font-medium', platformTextClass(group.platform)]">{{ group.name }}</span>
                    <span
                      :class="['rounded-sm px-1 py-0 text-[10px] tabular-nums', platformBadgeLightClass(group.platform)]"
                    >{{ group.rate_multiplier }}x</span>
                    <span class="text-[10px] tabular-nums text-fg-subtle">{{ group.account_count || 0 }}</span>
                    <span
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                ...
                <button type="button" @click="addMappingEntry(sIdx)" class="text-xs text-accent hover:text-accent-strong">
                  + {{ t('common.add', 'Add') }}
                </button>
              </div>
              <div
                v-if="Object.keys(section.model_mapping).length === 0"
                class="border-y border-border py-2 text-center text-xs text-fg-subtle"
              >
              ...
              <div class="mb-1 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  <button
                    ...
                    class="text-xs text-fg-muted hover:text-accent-strong disabled:opacity-50"
                  >
                    {{ syncingPlatform === section.platform ? t('admin.channels.form.syncingModels') : t('admin.channels.form.syncLatestModels') }}
                  </button>
                  <button type="button" @click="addPricingEntry(sIdx)" class="text-xs text-accent hover:text-accent-strong">
                    + {{ t('common.add', 'Add') }}
                  </button>
                </div>
              </div>
            ...
            <div class="mt-4 border-t border-border pt-4 space-y-3">
              ...
              <p
                ...
                class="text-xs italic text-fg-subtle"
              >
              ...
              >
                <div class="flex items-center justify-between">
                  <input
                    ...
                    class="bg-transparent text-label font-semibold text-fg placeholder-fg-subtle outline-none"
                  />
                  <button type="button" @click="removeAccountStatsRule(sIdx, ruleIndex)" class="text-xs text-danger hover:text-danger">
                    {{ t('common.delete') }}
                  </button>
                </div>

                <div>
                  <label class="text-xs text-fg-muted">{{ t('admin.channels.form.ruleGroups') }}</label>
                  <div class="mt-1 flex flex-wrap gap-1">
                    <label
                      v-for="gid in section.group_ids"
                      :key="gid"
                      class="inline-flex cursor-pointer items-center gap-1 rounded-sm border px-2 py-1 text-xs transition-colors"
                      :class="rule.group_ids.includes(gid)
                        ? 'border-accent bg-accent-weak'
                        : 'border-border hover:bg-accent-weak/50'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded border-border-strong text-accent focus:ring-accent" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="mt-1 text-xs text-fg-subtle">
                    {{ t('admin.channels.form.noGroupsInChannel') }}
                  </p>
                </div>

                <div>
                  <label class="text-xs text-fg-muted">{{ t('admin.channels.form.ruleAccounts') }}</label>
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    <label class="text-xs text-fg-muted">{{ t('admin.channels.form.ruleModelPricing') }}</label>
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="text-xs text-accent hover:text-accent-strong">
                      + {{ t('common.add') }}
                    </button>
                  </div>
                  <div v-if="rule.pricing.length === 0" class="border-y border-border py-2 text-center text-xs text-fg-subtle">
...
const columns = computed<Column[]>(() => [
  ...
  { key: 'group_count', label: t('admin.channels.columns.groups', 'Groups'), sortable: false, class: 'text-right' },
  { key: 'pricing_count', label: t('admin.channels.columns.pricing', 'Pricing'), sortable: false, class: 'text-right' },
...
.channel-tab {
  @apply -mb-px flex items-center gap-1.5 whitespace-nowrap border-b-2 px-3 py-2.5 text-label font-semibold transition-colors;
}

.channel-tab-active {
  @apply border-accent text-accent-strong;
}

.channel-tab-inactive {
  @apply border-transparent text-fg-muted hover:border-border-strong hover:text-accent-strong;
}
</style>
