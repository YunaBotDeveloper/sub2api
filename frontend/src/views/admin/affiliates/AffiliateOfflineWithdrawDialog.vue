  ...
  >
    <form class="space-y-4" @submit.prevent="submit">
      <div>
        ...
        <div
          v-if="selectedUser"
          class="flex items-center justify-between gap-3 border border-accent/40 bg-accent-weak px-3 py-2"
          data-test="withdraw-selected-user"
        >
          <div class="min-w-0 truncate text-sm">
            <span class="font-mono text-fg-muted">#{{ selectedUser.id }}</span>
            <span class="ml-2 font-medium text-fg">{{ selectedUser.email }}</span>
            <span v-if="selectedUser.username" class="ml-1 text-xs text-fg-muted">({{ selectedUser.username }})</span>
          </div>
          <button
            type="button"
            class="text-lg leading-none text-fg-subtle hover:text-danger disabled:opacity-50"
        ...
        <template v-else>
          ...
          <div
            ...
            class="mt-1 max-h-40 overflow-y-auto border border-border"
          >
            <button
              v-for="user in userResults"
              :key="user.id"
              type="button"
              class="w-full px-3 py-1.5 text-left text-sm text-fg hover:bg-surface-sunken"
              data-test="withdraw-user-option"
              @click="selectUser(user)"
            >
              {{ user.email }} <span class="text-xs text-fg-muted">({{ user.username }})</span>
            </button>
          </div>
      ...
      >
        <div class="flex items-center justify-between text-sm">
          <span class="text-fg-muted">{{ t('admin.affiliates.withdraw.availableQuota') }}</span>
          <span
            v-if="overviewLoading"
            class="h-4 w-4 animate-spin rounded-full border-2 border-accent border-t-transparent"
          ></span>
          <span v-else class="font-semibold tabular-nums text-fg" data-test="withdraw-available-quota">
            ${{ formatPreciseAmount(availableQuota) }}
          </span>
        </div>
        <p class="mt-1 text-xs text-fg-muted">{{ t('admin.affiliates.withdraw.frozenHint') }}</p>
      </div>
      ...
      <div
        ...
        class="border border-accent/40 bg-accent-weak p-3 text-sm text-accent-strong"
        data-test="withdraw-uncertain-hint"
      >
        {{ t('admin.affiliates.withdraw.uncertainHint') }}
      </div>

      <div class="border border-warning/40 bg-warning-weak p-3 text-sm text-warning-strong">
