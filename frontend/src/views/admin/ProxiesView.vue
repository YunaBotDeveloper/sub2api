  ...
  <AppLayout>
        ...
        >
          ...
          <template #cell-address="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                <button
                  type="button"
                  class="rounded-sm p-0.5 text-fg-subtle hover:text-accent"
                ...
                <div
                  ...
                  class="absolute left-0 top-full z-50 mt-1 w-auto min-w-[180px] rounded-sm border border-border bg-surface py-1 shadow-overlay"
                >
                  <button
                  ...
                  >
                    <span class="truncate font-mono text-fg-muted">{{ fmt.label }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              <div class="flex flex-col text-xs">
                <span v-if="row.username" class="text-fg">{{ row.username }}</span>
                <span v-if="row.password" class="font-mono text-fg-muted">
          ...
          <template #cell-location="{ row }">
            <div class="flex items-center gap-2">
              ...
              />
              <span v-if="formatLocation(row)" class="text-sm text-fg">
                {{ formatLocation(row) }}
              </span>
              <span v-else class="text-sm text-fg-subtle">-</span>
            </div>
          ...
          <template #cell-account_count="{ row, value }">
            <button
              v-if="(value || 0) > 0"
              type="button"
              class="badge badge-primary hover:border-accent"
          ...
          <template #cell-expiry="{ row }">
            <span v-if="!row.expires_at" class="text-sm text-fg-subtle">{{ t('admin.proxies.neverExpires') }}</span>
            <div v-else class="flex flex-col text-xs">
              <span class="text-fg">{{ formatDateTime(row.expires_at) }}</span>
              <span :class="expiryBadgeClass(row)">{{ expiryLabel(row) }}</span>
            </div>
          </template>

          <template #cell-created_at="{ row }">
            <span class="text-xs text-fg-muted">{{ formatDateTime(row.created_at) }}</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-fg-muted transition-colors hover:bg-success-weak hover:text-success disabled:cursor-not-allowed disabled:opacity-50"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-fg-muted transition-colors hover:bg-accent-weak hover:text-accent disabled:cursor-not-allowed disabled:opacity-50"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-fg-muted transition-colors hover:bg-surface-sunken hover:text-accent"
              >
    ...
    >
      ...
      <div v-else class="space-y-5">
        ...
        <!-- Parse Result -->
        <div v-if="batchParseResult.total > 0" class="border-t border-border pt-4">
    ...
    >
      <div v-if="qualityReport" class="space-y-4">
        <div class="border-b-2 border-accent pb-3">
          <div class="flex items-center justify-between gap-4">
            ...
            <div class="text-right">
              <div class="text-h1 font-bold tabular-nums text-fg">
                {{ qualityReport.score }}
              </div>
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-fg-muted">
        ...
        <div class="max-h-80 overflow-auto rounded-sm border border-border">
          <DataTable :columns="qualityReportColumns" :data="qualityReport.items" row-key="target">
            <template #cell-target="{ value }">
              <span class="text-fg">{{ qualityTargetLabel(value) }}</span>
            </template>
            <template #cell-status="{ value }">
