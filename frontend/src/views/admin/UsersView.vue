  ...
  <AppLayout>
                ...
                >
                  ...
                  >
                    ...
                    <Icon
                      ...
                      name="check"
                      size="sm"
                      :class="isForcedVisibleColumn(col.key) ? 'text-fg-subtle' : 'text-accent'"
                      :stroke-width="2"
                    />
                  </button>
                </div>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-notes="{ value }">
            <div class="max-w-xs">
              <span
                ...
                class="block truncate text-sm text-fg-muted"
              >
          ...
          >
            <div class="max-w-xs">
              <span
                class="block truncate text-sm text-fg"
          ...
          <template #cell-groups="{ row }">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                <Icon name="shield" size="xs" class="h-3.5 w-3.5 text-fg-muted" />
                <span class="font-semibold tabular-nums text-fg">{{ getUserGroups(row).exclusive.length }}</span>
                <span class="text-fg-muted">{{ t('admin.users.exclusiveLabel') }}</span>
                <!-- Hover tooltip（操作菜单未打开时显示） -->
                <div
                  v-if="expandedGroupUserId !== row.id"
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded-sm border border-border-strong bg-surface-raised px-2.5 py-1.5 text-xs text-fg opacity-0 shadow-overlay transition-opacity duration-75 group-hover/ex:opacity-100"
                >
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-border-strong"></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-sm border border-border-strong bg-surface-raised py-1 text-xs shadow-overlay"
                >
                  <div class="border-b border-border px-3 py-1.5 text-meta font-medium text-fg-muted">
                    {{ t('admin.users.clickToReplace') }}
                  </div>
                  <div
                    v-for="g in getUserGroups(row).exclusive"
                    :key="g.id"
                    class="flex cursor-pointer items-center gap-2 px-3 py-2 text-fg transition-colors hover:bg-accent-weak hover:text-accent-strong"
              ...
              >
                <Icon name="globe" size="xs" class="h-3.5 w-3.5 text-fg-subtle" />
                <span class="font-semibold tabular-nums text-fg">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="text-fg-subtle">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded-sm border border-border-strong bg-surface-raised px-2.5 py-1.5 text-xs text-fg opacity-0 shadow-overlay transition-opacity duration-75 group-hover/pub:opacity-100">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-border-strong"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 text-xs text-fg-subtle"
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center justify-end gap-2">
              <div class="group relative">
                <button
                  class="font-semibold tabular-nums text-fg underline decoration-border-strong decoration-dashed underline-offset-4 transition-colors hover:text-accent-strong"
                  @click="handleBalanceHistory(row)"
                >
                  ${{ value.toFixed(2) }}
                </button>
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded-sm border border-border-strong bg-surface-raised px-2 py-1 text-xs text-fg opacity-0 shadow-overlay transition-opacity duration-75 group-hover:opacity-100">
                  {{ t('admin.users.balanceHistoryTip') }}
                  <div class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-border-strong"></div>
                </div>
              </div>
              <button
                @click.stop="handleDeposit(row)"
                class="rounded-sm border border-border-strong px-2 py-0.5 text-xs font-semibold text-accent-strong transition-colors hover:border-accent hover:bg-accent-weak"
          ...
          <template #cell-balance_platform_quota="{ row }">
            <button
              type="button"
              class="block text-left underline decoration-border-strong decoration-dashed underline-offset-4 transition-colors hover:decoration-accent"
          ...
          >
            <div class="flex items-center gap-1.5">
              ...
              <div class="usage-sort-trigger relative">
                <button
                  type="button"
                  class="flex items-center gap-1 rounded-sm px-1 py-0.5 transition-colors hover:bg-surface"
                ...
                >
                  <button
                    ...
                    :key="metric"
                    type="button"
                    class="flex w-full items-center justify-between gap-3 px-3 py-1.5 text-left text-xs normal-case tracking-normal hover:bg-accent-weak"
                  ...
                  <div class="mt-1 border-t border-border px-3 py-1 text-[10px] normal-case tracking-normal text-fg-subtle">
          ...
          <template #cell-created_at="{ value }">
            <span class="text-sm tabular-nums text-fg-muted">{{ formatDateTime(value) }}</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :class="[
                  'flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-fg-muted transition-colors',
                  row.status === 'active'
                    ? 'hover:bg-warning-weak hover:text-warning'
                    : 'hover:bg-success-weak hover:text-success'
                ]"
              >
    ...
    <Teleport to="body">
      <div
        ...
        class="action-menu-content fixed z-[9999] w-48 overflow-hidden rounded-sm border border-border-strong bg-surface-raised shadow-overlay"
        ...
        <div class="py-1">
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              >
                <Icon name="key" size="sm" class="text-accent" :stroke-width="2" />
              ...
              <button
                ...
                class="dropdown-item w-full"
              >
                <Icon name="users" size="sm" class="text-accent" :stroke-width="2" />
              ...
              <button
                ...
                class="dropdown-item w-full"
              >
                <Icon name="chartBar" size="sm" class="text-accent" :stroke-width="2" />
              ...
              <button
                ...
                class="dropdown-item w-full"
              >
                <Icon name="dollar" size="sm" class="text-accent" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-danger hover:bg-danger-weak"
              >
...
const allColumns = computed<Column[]>(() => [
  ...
  { key: 'balance', label: t('admin.users.columns.balance'), sortable: true, class: 'text-right' },
